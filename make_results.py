...
if __name__ == '__main__':
    ...
    for dir in dirl:
        ...
        for nf in image_files:
            ...
            results = results.concat( pd.DataFrame( [ nf, np.mean(frac_icl_l), np.mean(frac_icl_l) - lowficl, upficl - np.mean(frac_icl_l) ], columns = [ 'name', 'ICL fraction', 'err up', 'err low' ]), ignore_index=True)
            break
            n_coregal = 3
